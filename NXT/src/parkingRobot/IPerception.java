...
public interface IPerception {
	...
	/**
	 * returns the groundcolortype of the left linesensor use
	 * <code>{@link IPerception#calibrateLineSensors}</code> to calibrate the
	 * sensors for given environmental brightness
	 * 
	...
	/**
	 * returns the groundcolortype of the right linesensor use
	 * <code>{@link IPerception#calibrateLineSensors}</code> to calibrate the
	 * sensors for given environmental brightness
	 * 
	 * @return groundtype (0-white ground, 1-grey ground, 2-black ground)
	...
	/**
	 * returns the brightness of the subsurfacecolor under the left linesensor
	 * in percent (0% for calibrated black ground; 100% for calibrated white
	 * ground). use <code>{@link IPerception#calibrateLineSensors}</code> to
	 * calibrate the sensors for given environmental brightness
	 * 
	...
	/**
	 * returns the brightness of the subsurfacecolor under the right linesensor
	 * in percent (0% for calibrated black ground; 100% for calibrated white
	 * ground). use <code>{@link IPerception#calibrateLineSensors}</code> to
	 * calibrate the sensors for given environmental brightness
	 * 
	...
	/**
	 * calibrate the two LineSensors use a black and white floor color
	 * 
	 */
	public void calibrateLineSensors();

	/**
	 * Displays all sensor data from the Arduino on the LCD-Display
	 * 
	 */
	...
	/**
	 * returns the Object of the {@link OdoSensor} class for the Navigation
	 * Module
	 * 
	...
	/**
	 * returns the latest measured shift in U-direction of the robot in mm. the
	 * related timedifference between the last 2 measurements returns
	 * {@link IPerception#getOdometryT}
	 * 
	...
	/**
	 * returns the latest measured shift in V-direction of the robot in mm. the
	 * related timedifference between the last 2 measurements returns
	 * {@link IPerception#getOdometryT}
	 * 
	...
	/**
	 * returns the Distance of the front Sensor to the next obstacle the Value
	 * commes from the first Tri-Sensor connected to the Arduino (the order of
	 * the Sensorports is set in the Arduino-code)
	 * 
	...
	/**
	 * returns the Distance of the front-side Sensor to the next obstacle the
	 * Value commes from the second Tri-Sensor connected to the Arduino (the
	 * order of the Sensorports is set in the Arduino-code)
	 * 
	...
	/**
	 * returns the Distance of the back Sensor to the next obstacle the Value
	 * commes from the third Tri-Sensor connected to the Arduino (the order of
	 * the Sensorports is set in the Arduino-code)
	 * 
	...
	/**
	 * returns the Distance of the back-side Sensor to the next obstacle the
	 * Value commes from the fourth Tri-Sensor connected to the Arduino (the
	 * order of the Sensorports is set in the Arduino-code)
	 * 
	...
	/**
	 * bundles both values relevant for measuring an angle difference by an
	 * encoder sensor
	 * 
	 * @author IfA
	 */
	public class AngleDifferenceMeasurement {
		...
		/**
		 * @param angleSum
		 *            sets angle that one wheel has turned since last request
		 */
		...
		/**
		 * @param deltaT
		 *            sets time between last an actual request
		 */
	...
	/**
	 * supports processing of encoder sensor by recording the angle difference
	 * since last call of <code>getEncoderMeasurement</code> and the
	 * corresponding time interval. For every encoder information receiver pair
	 * one specific IEncoderSensor object should be instantiated.
	 * 
	 * @author IfA
	 */
	public class EncoderSensor {
		...
		/**
		 * the angle that one wheel has turned since last request and the time
		 * between last an actual request. A new angle increment is added by
		 * {@link addAngle}. If this value is read by
		 * <code>getEncoderMeasurement</code> it is reset to zero.
		 */
		...
		/**
		 * Constructor of the {@link EncoderSensor} Class Construction of the
		 * class starts the meausrement
		 */
		...
		/**
		 * returns the angle that one wheel has turned since last request and
		 * the time between last an actual request bundled in an
		 * AngleDifferenceMeasurement object
		 * 
		 * @return <code>AngleDifferenceMeasurement.AngleSum</code> angle sum up
		 *         since last call in degrees,
		 *         <code>AngleDifferenceMeasurement.deltaT</code> time delta
		 *         since last call in milliseconds
		 */
		 ...
		 * 
		 * @param Angle
		 *            actual angle difference to be add to angle sum
		 */
	...
	/**
	 * bundles all values relevant for measuring an shift difference by an
	 * Odometry sensor
	 * 
	 * @author PMP
	 */
	public class OdoDifferenceMeasurement {
		...
		/**
		 * @param usum
		 *            sets shift difference in x-direcion since last request
		 */
		...
		/**
		 * @param vsum
		 *            sets shift difference in y-direcion since last request
		 */
	...
	/**
	 * supports processing of the Mouse-odometrysensor by recording the Shift in
	 * U-and V-direction since last call of <code>getOdoMeasurement</code> and
	 * the corresponding time interval. For every Sensor information receiver
	 * pair one specific OdoSensor object should be instantiated.
	 * 
	 * @author PMP
	 */
	public class OdoSensor {
		 ...
		 * between last an actual request. A new angle increment is added by
		 * {@link addAngle}. If this value is read by
		 * <code>getOdoMeasurement</code> it is reset to zero.
		 */
		IPerception.OdoDifferenceMeasurement odoDiffMeas = new IPerception.OdoDifferenceMeasurement();

		/**
		 * moment of time of last readout request - not used (timeinterval from
		 * arduino is used)
		 */
		long ReferenceTime;
		long CurrentTime;
		...
		/**
		 * Constructor of the {@link OdoSensor} Class Construction of the class
		 * starts the meausrement
		 */
		public OdoSensor() {
			CurrentTime = System.currentTimeMillis();
			ReferenceTime = System.currentTimeMillis();
		...
		/**
		 * returns the U-,V-Shift in mm since last request and the time between
		 * last an actual request bundled in an OdoDifferenceMeasurement object
		 * 
		 * @return <code>OdoDifferenceMeasurement.uSum</code> u-Shift since last
		 *         call in mm, <code>OdoDifferenceMeasurement.vSum</code>
		 *         v-Shift since last call in mm,
		 *         <code>OdoDifferenceMeasurement.deltaT</code> time delta since
		...
		/**
		 * adds the passed u- and v- shift and the timedifference between the
		 * last two measurements to the stored values
		 * 
		 * @param u
		 *            the given U-Shift in mm
		 * @param v
		 *            the given V-Shift in mm
		 * @param deltaT
		 *            the time between the last two measurements
		 */
