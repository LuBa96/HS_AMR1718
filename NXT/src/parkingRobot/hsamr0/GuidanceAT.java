...
//					lastDemo3Status = demo3Status.DEMO_INACTIVE;
			...
			// While action
			{
				...
				if (Button.ESCAPE.isDown()) {
					...
					currentStatus = CurrentStatus.PARK_THIS;
				} else if (Button.LEFT.isDown() || (hmi.getMode() == parkingRobot.INxtHmi.Mode.RESET)) {
					...
					currentStatus = CurrentStatus.DEMO3;
				} else if (hmi.getMode() == parkingRobot.INxtHmi.Mode.DEMO1) {
	...
	private static void followLineSubStateMachine(IControl control, INavigation navigation) {
		...
		case FOLLOW_LINE_STRAIGHT:
			...
			// distToMid = 0;
			if (wrongDir) {
				vectorA = map[navigation.getLineNumber()].getP1().subtract(map[navigation.getLineNumber()].getP2());
				vectorA.multiplyBy((float) 0.5);
				distToMidMax = vectorA.length();
				if (map[navigation.getLineNumber()].getP1().distance(currPose.getLocation()) > distToMidMax)
					vLine += 0.2;
				else
					vLine -= 0.2;
			} else {
		...
		case FOLLOW_LINE_RIGHT:
			...
			// while action
			phiDot = (-90 - (currPose.getHeading() - startPose.getHeading()) * (180 / Math.PI))
					/ (8 * timePeriod * 0.001);
			if (phiDot > -10)
				phiDot = -10;
			...
			// state transitions
			lastLineStatus = currLineStatus;
			if ((currPose.getHeading() - startPose.getHeading()) * (180 / Math.PI) <= -turnDeg) {
	...
	private static void parkThisSubStateMachine(IControl control, INavigation navigation) {
		...
		case PARK_LINE_FOLLOW:
			...
			// while action
			RConsole.println("Status PARK_LINE_FOLLOW, mapGoalDist: " + currPose.getLocation().distance(mapGoal));
			followLineSubStateMachine(control, navigation);
		...
		case PARK_PATH_FOLLOW:
			...
			if (lastParkStatus != currParkStatus) {
				...
				coEffs = setPolynomial(coSys.getTransformedPose(currPose).getLocation());
				direction = 1;
			...
			// while action
			RConsole.println("Status PARK_LINE_FOLLOW, slotGoalDist: " + currPose.getLocation().distance(slotGoal));
			RConsole.println("slotGoal: " + slotGoal.getX() + " , " + slotGoal.getY());
			computePhiDot(coSys.getTransformedPose(currPose), coEffs);
			// velocity control, the straighter the path the faster the robot
			vPark = vParkMax * (1 - Math.abs(deltaPhiDeg) / 30);
			if (vPark < 5)
				vPark = 5;
			phiDot = phiDot * (vPark / vParkMax);
			control.setAngularVelocity(phiDot);
	...
	private static void demo1SubStateMachine(IControl control, INavigation navigation) {
		...
		case DEMO_LINE_CONTROL:
			...
			lastDemo1Status = currDemo1Status;
			if (control.getRightTurn()) {
				if ((currPose.getLocation().distance(map[0].getP1()) / map[0].length() <= 0.4)) {
					demo1Fin = true;
				}
			}
			// leave action
	...
	private static void demo2SubStateMachine(IControl control, INavigation navigation) {
	...
	private static void demo3SubStateMachine(IControl control, INavigation navigation) {
