...
public class NavigationAT implements INavigation{
	 ...
	 */
	IPerception.AngleDifferenceMeasurement angleMeasurementLeft 	= 	null; //Unterschied zu encoderSensor?? zus�tzlich noch Zeitdifferenz?
	/**
	 ...
	 * F�r Numerierung der Kurvenpunkte, siehe Dropbox Ordner Navigation -> Kurvenpunktfestlegung**/
	public int aktuellerKurvenpunkt = 7; //Startpunkt des Roboters entspricht Kurvenpunkt 7
	
	
	/**fusionMatrix wird fuer fusionsFunktion der Pose benoetigt. Die Zeilen der Matrix stehen f�r die verschiedenen Sensoren wobei:Encorder (0), Maussensor (1), Kompass (2)
	...
	/**Kurvenmatrix in der alle Kurvenpunkte sowie deren Toleranzbereich f�r die Zuordnung der aktuellen Position zu einem Kurvenpunkt gespeichert sind
	...
	INavigation.ParkingSlot[] parkingSlots = new INavigation.ParkingSlot[10]; //10 Parkluecken mit Index 0 - 9 ansprechbar
...
//	private boolean frontBoundaryDetektiert = false; //wird benoetigt um backBoundary nicht auf gleichem Streckenabschnitt erneut neu zu setzen
	...
	public NavigationAT(IPerception perception, IMonitor monitor){
		...
		parkingSlots[0] = new ParkingSlot(0,Point1, Point2, ParkingSlotStatus.NOT_SUITABLE_FOR_PARKING, 0 );
		parkingSlots[1] = new ParkingSlot(0,Point1, Point2, ParkingSlotStatus.NOT_SUITABLE_FOR_PARKING, 0 );
		parkingSlots[2] = new ParkingSlot(0,Point1, Point2, ParkingSlotStatus.NOT_SUITABLE_FOR_PARKING, 0 );
		parkingSlots[3] = new ParkingSlot(0,Point1, Point2, ParkingSlotStatus.NOT_SUITABLE_FOR_PARKING, 0 );
		parkingSlots[4] = new ParkingSlot(0,Point1, Point2, ParkingSlotStatus.NOT_SUITABLE_FOR_PARKING, 0 );
		parkingSlots[5] = new ParkingSlot(0,Point1, Point2, ParkingSlotStatus.NOT_SUITABLE_FOR_PARKING, 0 );
		parkingSlots[6] = new ParkingSlot(0,Point1, Point2, ParkingSlotStatus.NOT_SUITABLE_FOR_PARKING, 0 );
		parkingSlots[7] = new ParkingSlot(0,Point1, Point2, ParkingSlotStatus.NOT_SUITABLE_FOR_PARKING, 0 );
		parkingSlots[8] = new ParkingSlot(0,Point1, Point2, ParkingSlotStatus.NOT_SUITABLE_FOR_PARKING, 0 );
		parkingSlots[9] = new ParkingSlot(0,Point1, Point2, ParkingSlotStatus.NOT_SUITABLE_FOR_PARKING, 0 );
...
//		last_y = y_in;
		...
		heading += dheading; //zum Testen heading einfach mit dx und dy vorgeben, update: jetzt mit dx und last_dx bestimmen
			//Ueberlaufe verhindern //fuer cos, sin Funktionen f�r 'Position bestimmen' Ueberlauf nicht schlimm. Nur fuer Schoenheit
	...
	private void fusionOfPoseData(double s, double x, double y, double angle) {
		 ...
		 * Mit WinkelkorrekturZwischenEcken() wird zuerst der Mittelwert der Abweichung der Blickrichtung (heading) gebildet und diese anschlie�end auf ungefaehr halber Strecke zwischen zwei Ecken zum aktuellen Blickwinkel aufaddiert/abgezogen.
...
//					frontBoundaryDetektiert = false;
	 ...
	 * Die Methode ermittelt die vorderen Positionsecken der Parkluecke.
	 * Fuer Guidance wird es eine extra Methode geben welche die Positionen auf halber Tiefe der Parkluecken augibt **/
	private void detectParkingSlot(){
	
		double aktuellFrontSideSensorDistance = perception.getFrontSideSensorDistance();
		boolean bedingung1 = aktuellerKurvenpunkt != 1 && aktuellerKurvenpunkt != 2 && aktuellerKurvenpunkt != 4 && aktuellerKurvenpunkt != 5 && aktuellerKurvenpunkt != 6; //hier existieren keine Parkluecken
	...
	private void backBoundarx_ySetzen() {
		...
		case 0: backBoundarxFrontSensor = this.pose.getX()*100 + DISTANCE_RobotCenterToBarrierK0K7; //y Position gut, fuer x Position evt. ein spaeteren Wert nehmen, da zu frueh ermittelt und evt noch an Kante
				...
				/**Auf diesen Strecken gibt es keine Parkluecken. In parklueckeHinzufuegen Methode werden auf diesen Strecken keine Parkluecken hinzugefuegt.
				 * Falls doch, dann mit den Werten 0,0**/
	...
	private void frontBoundarx_ySetzen() {
		frontBoundarxFrontSensor = frontBoundarxFrontSensorAktuell; //x Koordinate vom vorderen Parklueckenpunkt wird gespeichert; ACHTUNG hier wird frontBoundary
	...
	private void parklueckeHinzufuegenOderAktualisieren() {
		...
		switch(aktuellerKurvenpunkt) {
			case 0: parklueckenLaenge = frontBoundaryFrontSensor - backBoundaryFrontSensor;
					if (parklueckenLaenge > MIN_SLOT_DISTANCE) { //Parkluecke soll vorerst mindestens 45cm gro� sein
...
/**TODO:
	...
	9. PowerPoint vorbereiten
	10. AktualisierenMethode schreiben
