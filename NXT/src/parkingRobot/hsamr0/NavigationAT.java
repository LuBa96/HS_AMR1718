...
public class NavigationAT implements INavigation {
	...
	static final double DISTANCE_FrontSideSensorToRobotCenter = 2; // in cm,
																	// noch
																	// ausmessen!
																	// (TODO)
	...
	static final double DISTANCE_RobotCenterToBarrier = 15; // in cm; Wert
															// muesste stimmen,
															// aber nochmal
															// ueberpruefen
															// (TODO)
	...
	/**
	 * Kurvenmatrix in der alle Kurvenpunkte sowie deren Toleranzbereich für die
	 ...
	 * yminBereich, ymaxBereich, xKurvenpunkt, yKurvenpunkt, Einfahrtwinkel in
	 * die nach dem aktuellen Kurvenpunkt folgende Kurve
	 **/
	double[][] Kurvenmatrix = new double[8][8];

	/**
	 * Datenbank fuer Parkluecken Spaltenformat: Parklueckenindex (beginnend bei
	 * 0), hinterer Punkt Parkluecke, vorderer Punkt Parkluecke -> Definition
	 * von hinterem und vorderem Punkt: Steht Roboter neben Parkluecke, befindet
	 * sich der hintere Punkt der Parkluecke in Fahrtrichtung gesehen hinter dem
	 * Roboter
	 **/
		...
		if ((!this.off_track && aktuellInKurve)) { // && aktuellInKurve
			...
			for (int i = 0; i < 8; i++) {
				if ((aktuellInKurve && xResult * 100 > Kurvenmatrix[i][1])
					...
					/**
					 * sobald Kurve beendet, befindet man sich optimalerweise
					 * wieder in Geradeaus-Fahrt -> Blickrichtung koennte hier
					 * evt. korrigiert werden oder Kalibrierung fuer Kompass
					 * waere hier moeglich
					 **/

				} else {
					/**
					 * Hier evt. Fehlerkorrektur (vorerst nicht wichtig) Info:
					 * Man gelangt in Schleife: - bei 7 von 8 Durchlaeufen, da
					 * aktuelle Position nur einem Bereich zugeordnet werden
					 * kann (1x If schleife, 7x else schleife) - wenn man sich
					 * angeblich in Kurve befindet, der aktuellen Position aber
					 * kein Kurvenpunkt zugeordnet werden kann -> vorerst wird
					 * an aktueller Position keine Aenderung vorgenommen (else
					 * Schleife bleibt leer)
					 **/
				}
			}
		}
	 ...
	 * gesetzt Die Methode ermittelt nicht die vorderen Positionsecken der
	 * Parkluecke sondern die Positionen auf halber Tiefe in den Parkluecken!
	 **/
	private void detectParkingSlot() {

		double aktuellFrontSideSensorDistance = perception
				.getFrontSideSensorDistance();
		if (aktuellFrontSideSensorDistance > 15) { // ACHTUNG: Korrekt so auf
													// perception zuzugreifen??
													// Hier die Tiefe angeben
													// die Parkluecke mindestens
													// haben muss
			switch (aktuellerKurvenpunkt) {
			case 0: // Roboter faehrt in positive y Richtung
				backBoundarxFrontSensor = this.pose.getX()
						* 100
						+ (aktuellFrontSideSensorDistance - DISTANCE_RobotCenterToBarrier)
						/ 2; // y Position gut, fuer x Position evt. ein
								// spaeteren Wert nehmen, da zu frueh ermittelt
								// und evt noch an Kante
				backBoundaryFrontSensor = this.pose.getY() * 100
						+ DISTANCE_FrontSideSensorToRobotCenter;
			case 2:
			case 6: // Roboter faehrt in negative y Richtung
